...
● 브랜드명이 일반 명사·고유명사와 우연히 동일/유사한 경우, 그 일반 명사 본래 의미에서
  연상되는 키워드는 절대 생성하지 않는다 — 이 규칙은 광고주 브랜드명뿐 아니라
  경쟁사 브랜드명에도 동일하게 적용한다
  (예: 브랜드 "성경" → 종교 서적 "성경(Bible)"과 무관 → "성경말씀", "성경공부", "성경구절",
   "종교", "통일교", "교회" 등 종교 연상 키워드 완전 금지. 브랜드 "미소"→"미소짓다" 등
   일반 감정·동사 표현 금지. 경쟁사 브랜드 "에코"→ 일반 명사 "에코(친환경/eco)"와 무관
   → "에코패션", "에코백", "에코디자이너" 등 친환경 연상 키워드 완전 금지, 신발 브랜드
   문맥에 맞는 키워드만 생성)
● SA 전략 인사이트에 "제외 패턴"으로 명시된 키워드 유형·단어는 절대 생성하지 않는다 —
  인사이트는 참고용이 아니라 반드시 지켜야 할 제약 조건이다
...
→ 중요: 이 카테고리의 대표 키워드는 반드시 "상위(포괄적) 개념"과 "하위(세부) 개념"을
  함께 포함한다 — 세부 개념만 좁게 잡으면 이후 연관 키워드 확장이 그 좁은 범위 안에서만
  이뤄져 실제 검색량 있는 인접 키워드를 놓친다.
  (예: 광고주 제품이 "조미김"이어도 general 카테고리는 "조미김"에만 국한하지 말고,
   더 상위 개념인 "김"까지 포함해서 "김", "구운김", "재래김", "김 선물세트" 등
   상위·하위를 고루 섞는다. 광고주 제품이 "무선 이어폰"이면 "이어폰"까지 포함)

[competitor 타입] 선택 1개 — {competitors_str} 검색자 전환 유도, 10~25개(목표치, 상한 아님)
→ 경쟁사 목록에 있는 브랜드는 하나도 빠짐없이 전부 포함한다 (일부만 골라 쓰지 않는다).
  경쟁사별로 최소 1개 이상은 반드시 생성하고, 그 위에서 경쟁사당 최소 4개를 목표로 한다
→ 각 경쟁사 + {category}/모델/가격/추천/비교/구매 (실제 검색될 법한 조합만)
