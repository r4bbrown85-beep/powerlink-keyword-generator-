...
[brand 타입] 필수 1개 — {brand} 직접 검색 사용자, 8~15개 (품질 우선, 초과 금지)
...
[product 타입] 권장 1~2개 — {products_str} 구매 직전 소비자, 각 15~30개(목표치, 상한 아님)
→ 브랜드 + 모델명/스펙/용도/특성 조합
→ 구체적 제품명·서비스명 중심 (한글·영문·혼용 표기 포함)
→ 제외: 이 캠페인 제품 외 {brand}의 다른 제품
→ 실제 검색량이 있을 법한 표현이 15개가 안 되면 그만큼만 생성한다

[general 타입] 권장 1개 — {category} 카테고리 탐색 잠재 구매자, 15~30개(목표치, 상한 아님)
